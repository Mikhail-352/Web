...
function priority(operation: string): number {
...
function isOperation(str: string): boolean {
    return /^[\+\-\*\/]{1}$/.test(str)
}
...
function evaluate(str: string): number | string {
    ...
    for (let token of tokens) {
        if (isNumeric(token)) {
            ...
            const operand2 = stack.pop() as number; // Правое число
            const operand1 = stack.pop() as number; // Левое число
...
window.onload = function () {
    document.querySelector('.buttons')?.addEventListener('click', function (event: Event) {
    ...
    document.addEventListener('keydown', function (event: KeyboardEvent) {
        ...
        if (event.key >= '0' && event.key <= '9') { // Если нажата цифровая клавиша или точка
            ...
            if (screen.textContent) screen.textContent = screen.textContent.slice(0, -1)
        }
    })
}
