...
const api_key = "8ac9a581-7d54-4db4-8bc3-8ffce520477c";
...
dataTourForm.addEventListener('input', function () {
    ...
    var russianHolidays = [
        ...
        new Date('2024-01-07'),   // Christmas Day
...
function additionalOptions(isPensioner, isSignTranslator, numberOfPeople, startMonet) {
    if (isPensioner < 0 || isSignTranslator < 0) {
    ...
    if (isPensioner) {
        price = startMonet * 0.75;
    }

    if (isSignTranslator) {
        if (numberOfPeople >= 0 || numberOfPeople <= 5){
            price = startMonet * 1.15;
        }
        if (numberOfPeople >= 6 || numberOfPeople <= 10){
            price = startMonet * 1.25;
        }
        if (numberOfPeople >= 11){
            throw new Error('Слишком много людей');
        }
    }
